...
#%%
spatiotemporal_train_result=pd.read_excel("result\performance_spatiotemporal_transformer-lstm(RH)-new.xlsx",sheet_name="train").iloc[:,1:16]
spatiotemporal_test_result=pd.read_excel("result\performance_spatiotemporal_transformer-lstm(RH)-new.xlsx",sheet_name="test").iloc[:,1:16]
...
# new_unique_index = np.array(new_unique_index)
...
btm_info=pd.read_csv(r"D:\lab\research\weather downscale\zhuoshui\gis\btm.txt"); btm_info=btm_info.loc[:,'TOWNNAME']
mid_info=pd.read_csv(r"D:\lab\research\weather downscale\zhuoshui\gis\mid.txt"); mid_info=mid_info.loc[:,'TOWNNAME']
top_info=pd.read_csv(r"D:\lab\research\weather downscale\zhuoshui\gis\top.txt"); top_info=top_info.loc[:,'TOWNNAME']
...
writer = pd.ExcelWriter(r'result\seasonal_performance(T+%s).xlsx'%forecast_timestep,engine='xlsxwriter')
for season_index in range(len(season)):
    ...
    selected_forecast_mae=pd.DataFrame([error_indicator.np_mae(pd.DataFrame(spatiotemporal_result.iloc[new_unique_index[index],3+forecast_timestep-1]).reset_index(drop=True).T,
    ...
    selected_simulate_mae=pd.DataFrame([error_indicator.np_mae(pd.DataFrame(spatiotemporal_result.iloc[new_unique_index[index],3+forecast_timestep-1]).reset_index(drop=True).T,
    ...
    Plot figures
    """
    create_file(r"event\season\T+%s"%(forecast_timestep))
    cmap = plt.cm.get_cmap('Blues')
    # min_val = 20; max_val=spatiotemporal_result.iloc[:,3:].max().max()
    min_val = 60; max_val=100
    ...
    plt.savefig(r"event\season\T+%s\forecast_%s.png"%(forecast_timestep,season[season_index]))
    gc.collect()
    ...
    gdf.plot(ax=ax, edgecolor='black', facecolor='none', linewidth=0.5)
    cbar = fig.colorbar(plt.cm.ScalarMappable(norm=norm, cmap=cmap), ax=ax, ticks=np.linspace(min_val, max_val, 10))
    plt.savefig(r"event\season\T+%s\obs_%s.png"%(forecast_timestep,season[season_index]))
    gc.collect()
    ...
    gdf.plot(ax=ax, edgecolor='black', facecolor='none', linewidth=0.5)
    cbar = fig.colorbar(plt.cm.ScalarMappable(norm=norm, cmap=cmap), ax=ax, ticks=np.linspace(min_val, max_val, 10))
    plt.savefig(r"event\season\T+%s\simulate_%s.png"%(forecast_timestep,season[season_index]))
    gc.collect()    
    ...
    plt.savefig(r"event\season\T+%s\std_%s.png"%(forecast_timestep,season[season_index]))
    gc.collect()
    
    #plot error figure
    min_error = 0; max_error=20
    ...
    plt.savefig(r"event\season\T+%s\error_%s.png"%(forecast_timestep,season[season_index]))
    gc.collect()
    ...
    gdf.plot(ax=ax, edgecolor='black', facecolor='none', linewidth=0.5)
    cbar = fig.colorbar(plt.cm.ScalarMappable(norm=norm2, cmap=cmap2), ax=ax, ticks=np.linspace(min_error, max_error, 10))
    plt.savefig(r"event\season\T+%s\simulate_error_%s.png"%(forecast_timestep,season[season_index]))
    gc.collect()
...
writer = pd.ExcelWriter(r'result\seasonal_performance(each_year)-T+%s.xlsx'%forecast_timestep,engine='xlsxwriter')
...
for year_index in range(len(all_indices)):
    ...
    for season_index in range(len(all_indices[year_index])):
        ...
        #calculate the absolute error
        selected_simulate_mae=pd.DataFrame([error_indicator.np_mae(pd.DataFrame(spatiotemporal_result.iloc[new_unique_index[index],3+forecast_timestep-1]).reset_index(drop=True).T,pd.DataFrame(simulate.iloc[new_unique_index[index],-1]).reset_index(drop=True).T) for index in indices[season_index]]).mean()
        selected_simulate_mae_=pd.concat([station_info,selected_simulate_mae.astype(float)],axis=1); selected_simulate_mae_.columns=['TOWNNAME','value']
        ...
        gdf.plot(ax=ax, edgecolor='black', facecolor='none', linewidth=0.5)
        cbar = fig.colorbar(plt.cm.ScalarMappable(norm=norm, cmap=cmap), ax=ax, ticks=np.linspace(min_val, max_val, 10))
        plt.savefig(r"event\season\T+%s\forecast_%s(%s).png"%(forecast_timestep,season[season_index],years[year_index]))
        gc.collect()
        ...
        gdf.plot(ax=ax, edgecolor='black', facecolor='none', linewidth=0.5)
        cbar = fig.colorbar(plt.cm.ScalarMappable(norm=norm, cmap=cmap), ax=ax, ticks=np.linspace(min_val, max_val, 10))
        plt.savefig(r"event\season\T+%s\obs_%s(%s).png"%(forecast_timestep,season[season_index],years[year_index]))
        gc.collect()
        ...
        gdf.plot(ax=ax, edgecolor='black', facecolor='none', linewidth=0.5)
        cbar = fig.colorbar(plt.cm.ScalarMappable(norm=norm, cmap=cmap), ax=ax, ticks=np.linspace(min_val, max_val, 10))
        plt.savefig(r"event\season\T+%s\simulate_%s(%s).png"%(forecast_timestep,season[season_index],years[year_index]))
        gc.collect()        
        ...
        gdf.plot(ax=ax, edgecolor='black', facecolor='none', linewidth=0.5)
        cbar = fig.colorbar(plt.cm.ScalarMappable(norm=norm, cmap=cmap), ax=ax, ticks=np.linspace(min_val, max_val, 6))
        plt.savefig(r"event\season\T+%s\std_%s(%s).png"%(forecast_timestep,season[season_index],years[year_index]))
        gc.collect()
        ...
        gdf.plot(ax=ax, edgecolor='black', facecolor='none', linewidth=0.5)
        cbar = fig.colorbar(plt.cm.ScalarMappable(norm=norm2, cmap=cmap2), ax=ax, ticks=np.linspace(min_error, max_error, 10))
        plt.savefig(r"event\season\T+%s\error_%s(%s).png"%(forecast_timestep,season[season_index],years[year_index]))
        gc.collect()
        ...
        gdf.plot(ax=ax, edgecolor='black', facecolor='none', linewidth=0.5)
        cbar = fig.colorbar(plt.cm.ScalarMappable(norm=norm2, cmap=cmap2), ax=ax, ticks=np.linspace(min_error, max_error, 10))
        plt.savefig(r"event\season\T+%s\simulate_error_%s(%s).png"%(forecast_timestep,season[season_index],years[year_index]))
        gc.collect()
...
zero_stas=pd.read_excel(r'result\seasonal_performance(each_year)-T+%s.xlsx'%forecast_timestep,sheet_name="2020_subcatchment",index_col=0)
one_stas=pd.read_excel(r'result\seasonal_performance(each_year)-T+%s.xlsx'%forecast_timestep,sheet_name="2021_subcatchment",index_col=0)
two_stas=pd.read_excel(r'result\seasonal_performance(each_year)-T+%s.xlsx'%forecast_timestep,sheet_name="2022_subcatchment",index_col=0)
three_stas=pd.read_excel(r'result\seasonal_performance(each_year)-T+%s.xlsx'%forecast_timestep,sheet_name="2023_subcatchment",index_col=0)
...
stas.to_csv(r'result\seasonal_statistics_comparison-T+%s.csv'%forecast_timestep)
