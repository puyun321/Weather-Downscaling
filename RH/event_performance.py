...
spatiotemporal_train_result=pd.read_excel("result\performance_spatiotemporal_transformer-lstm(RH).xlsx",sheet_name="train").iloc[:-2,1:16]
spatiotemporal_test_result=pd.read_excel("result\performance_spatiotemporal_transformer-lstm(RH).xlsx",sheet_name="test").iloc[:-2,1:16]
...
for timestep in range(1,7):
    array=pd.read_excel(r'result\event_performance(test).xlsx',sheet_name="region_mae(T+%s)"%timestep,index_col=0)
    for i in range(len(array)):
        new_array=pd.concat([unique_station_info,array.iloc[i,1:].astype(float).reset_index(drop=True)],axis=1)
        new_array.columns=['TOWNNAME','value']
