...
spatiotemporal_train_result=pd.read_excel("result\performance_spatiotemporal_transformer-lstm(RH).xlsx",sheet_name="train").iloc[:,1:16]
spatiotemporal_test_result=pd.read_excel("result\performance_spatiotemporal_transformer-lstm(RH).xlsx",sheet_name="test").iloc[:,1:16]
...
simulate_result=simulate.iloc[:,-1]

station_info=simulate.iloc[:,:3]
...
#%% get maximum relative humidity event
...
# Create a colormap
min_val=60; max_val=np.max(spatiotemporal_result.iloc[:,3:15])

norm = Normalize(vmin=min_val, vmax=max_val)
cmap = plt.cm.get_cmap('Blues')
sm = ScalarMappable(norm=norm, cmap=cmap)

def plot_regional_result(region_info,array,date_index, train_index=0):
    ...
    if train_index==0:
        ...
        plt.savefig(r"event\train\%s\simulate_t+6.png"%(date[date_index][0:13]))

    else:
        date_index=date_index
        create_file(r"event\test\%s"%date[date_index][0:13])
        plt.savefig(r"event\test\%s\simulate_t+6.png"%(date[date_index][0:13]))
...
for train_index_ in range(len(regional_mean_max_train_index)):
    ...
    plot_regional_result(region_info,(simulate.iloc[unique_date_index[index],:]),index, 0)

for test_index_ in range(48,len(regional_mean_max_test_index)):
    index= test_index_+len(regional_mean_max_train_index)
    plot_regional_result(region_info,(simulate.iloc[unique_date_index[index],:]),index, 1)
...
for date_index in regional_mean_max_train_index:
    ...
    for timestep in range(1,7):
        ...
        simulate_=pd.DataFrame(simulate_result.iloc[unique_date_index[date_index]])
        r2[timestep-1].append(ErrorIndicator.np_R2(obs, simulate_))
        rmse[timestep-1].append(ErrorIndicator.np_RMSE(obs, simulate_))
        mae[timestep-1].append(ErrorIndicator.np_mae(obs, simulate_)[0])
        region_mae[timestep-1].append([ErrorIndicator.np_mae(obs.iloc[index,0], simulate_.iloc[index])[0]  for index in range(obs.shape[0])])
        top_mae[timestep-1].append(ErrorIndicator.np_mae(obs.iloc[top_index,0], simulate_.iloc[top_index])[0])
        mid_mae[timestep-1].append(ErrorIndicator.np_mae(obs.iloc[mid_index,0], simulate_.iloc[mid_index])[0])
        btm_mae[timestep-1].append(ErrorIndicator.np_mae(obs.iloc[btm_index,0], simulate_.iloc[btm_index])[0])
...
writer = pd.ExcelWriter(r'result\event_performance(train)-simulate.xlsx',engine='xlsxwriter')
...
writer = pd.ExcelWriter(r'result\event_performance(test)-simulate.xlsx',engine='xlsxwriter')
r2_.to_excel(writer,sheet_name="r2")
rmse_.to_excel(writer,sheet_name="rmse")
mae_.to_excel(writer,sheet_name="mae")
top_mae_.to_excel(writer,sheet_name="top_mae")
...
train1=pd.read_excel(r'result\event_performance(train)-simulate.xlsx',sheet_name="region_mae(T+1)",index_col=0)
train2=pd.read_excel(r'result\event_performance(train)-simulate.xlsx',sheet_name="region_mae(T+2)",index_col=0)
train3=pd.read_excel(r'result\event_performance(train)-simulate.xlsx',sheet_name="region_mae(T+3)",index_col=0)

test1=pd.read_excel(r'result\event_performance(test)-simulate.xlsx',sheet_name="region_mae(T+1)",index_col=0)
test2=pd.read_excel(r'result\event_performance(test)-simulate.xlsx',sheet_name="region_mae(T+2)",index_col=0)
test3=pd.read_excel(r'result\event_performance(test)-simulate.xlsx',sheet_name="region_mae(T+3)",index_col=0)
...
for timestep in range(1,7):
    array=pd.read_excel(r'result\event_performance(train)-simulate.xlsx',sheet_name="region_mae(T+%s)"%timestep,index_col=0)
    for i in range(len(array)):
        new_array=pd.concat([unique_station_info,array.iloc[i,1:].astype(float).reset_index(drop=True)],axis=1)
        new_array.columns=['TOWNNAME','value']
        create_file(r"event\train_simulate_error\%s"%array.iloc[i,0][0:13])
        # Perform the join
        ...
        plt.savefig(r"event\train_simulate_error\%s\t+%s_error.png"%(array.iloc[i,0][0:13], timestep))
        
for timestep in range(1,7):
    array=pd.read_excel(r'result\event_performance(test)-simulate.xlsx',sheet_name="region_mae(T+%s)"%timestep,index_col=0)
    for i in range(len(array)):
        
        new_array=pd.concat([unique_station_info,array.iloc[i,1:].astype(float).reset_index(drop=True)],axis=1)
        new_array.columns=['TOWNNAME','value']
        create_file(r"event\test_simulate_error\%s"%array.iloc[i,0][0:13])
        # Perform the join
        joined_gdf = gdf.merge(new_array, on='TOWNNAME')
        # Plot the shapefile
        fig, ax = plt.subplots(figsize=(10, 10))
        ...
        cbar = fig.colorbar(plt.cm.ScalarMappable(norm=norm, cmap=cmap), ax=ax, ticks=np.linspace(min_val, max_val, 5))
        plt.savefig(r"event\test_simulate_error\%s\t+%s_error.png"%(array.iloc[i,0][0:13], timestep))
