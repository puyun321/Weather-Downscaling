...
def simulate_evaluation(simulate, obs):
    ...
    for i in range(0,obs.shape[1]):
        rmse[i].append(ErrorIndicator.RMSE(simulate, obs[:,i]))
        r2[i].append(ErrorIndicator.R2(simulate,obs[:,i]))
        rae[i].append(ErrorIndicator.RAE(simulate,obs[:,i]))
...
writer = pd.ExcelWriter(r'T\result\performance_simulate.xlsx',engine='xlsxwriter')
#save performance
