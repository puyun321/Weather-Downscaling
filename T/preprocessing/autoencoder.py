...
#%%
obs_path=r"image_dataset/T/obs(T-0)"
obs_path_dir=os.listdir(obs_path)[:-1]

simulate_path=r"image_dataset/T/simulate"
simulate_path_dir=os.listdir(simulate_path)[:-1]

#%% read image
import cv2

obs_T=[cv2.resize(cv2.cvtColor(cv2.imread(obs_path+'\\'+image_path), cv2.COLOR_BGR2GRAY), (180, 180)) for image_path in obs_path_dir]
simulate_T=[cv2.resize(cv2.cvtColor(cv2.imread(simulate_path+'\\'+image_path), cv2.COLOR_BGR2GRAY), (180, 180)) for image_path in simulate_path_dir]
...
train_index=np.array([i for i in range(int(len(obs_T)*(train_ratio+val_ratio)))])
test_index=np.array([i for i in range(int(len(obs_T)*(train_ratio+val_ratio)),len(obs_T))])

# train
train_data_1=[obs_T[index] for index in train_index]
train_data_2=[simulate_T[index] for index in train_index]
train_data=np.concatenate([train_data_1,train_data_2])

# test
test_data_1=[obs_T[index] for index in test_index]
