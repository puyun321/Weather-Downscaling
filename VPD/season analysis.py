...
# new_unique_index  is the index with all date
...
#%%
def seasonal_performance(forecast_timestep):
    ...
    writer2 = pd.ExcelWriter(r'result\seasonal_performance(each_year)-T+%s.xlsx'%forecast_timestep,engine='xlsxwriter')
    ...
    for season_index in range(len(season)):
        ...
        Plot figures
        """
        create_file(r"image_result\season\T+%s"%(forecast_timestep))
        cmap = plt.cm.get_cmap('Purples')
        min_val = 0; max_val=3.5
        ...
        plt.savefig(r"image_result\season\T+%s\forecast_%s.png"%(forecast_timestep,season[season_index]))
        gc.collect()
        ...
        gdf.plot(ax=ax, edgecolor='black', facecolor='none', linewidth=0.5)
        cbar = fig.colorbar(plt.cm.ScalarMappable(norm=norm, cmap=cmap), ax=ax, ticks=np.linspace(min_val, max_val, 10))
        plt.savefig(r"image_result\season\T+%s\obs_%s.png"%(forecast_timestep,season[season_index]))
        gc.collect()
        ...
        gdf.plot(ax=ax, edgecolor='black', facecolor='none', linewidth=0.5)
        cbar = fig.colorbar(plt.cm.ScalarMappable(norm=norm, cmap=cmap), ax=ax, ticks=np.linspace(min_val, max_val, 10))
        plt.savefig(r"image_result\season\T+%s\simulate_%s.png"%(forecast_timestep,season[season_index]))
        gc.collect()    
        ...
        plt.savefig(r"image_result\season\T+%s\std_%s.png"%(forecast_timestep,season[season_index]))
        gc.collect()
        
        #plot error figure
        min_error = 0.0; max_error=1
        ...
        plt.savefig(r"image_result\season\T+%s\error_%s.png"%(forecast_timestep,season[season_index]))
        gc.collect()
        ...
        gdf.plot(ax=ax, edgecolor='black', facecolor='none', linewidth=0.5)
        cbar = fig.colorbar(plt.cm.ScalarMappable(norm=norm2, cmap=cmap2), ax=ax, ticks=np.linspace(min_error, max_error, 10))
        plt.savefig(r"image_result\season\T+%s\simulate_error_%s.png"%(forecast_timestep,season[season_index]))
        gc.collect()
        ...
        for year_index in range(len(all_indices)):
            ...
            for season_index in range(len(all_indices[year_index])):
                ...
                selected_obs_array_=pd.concat([station_info,selected_obs_array],axis=1); selected_obs_array_.columns=['TOWNNAME','value']
                selected_obs_std=pd.concat([pd.DataFrame(spatiotemporal_result.iloc[new_unique_index[index],3+forecast_timestep-1]).reset_index(drop=True).T for index in season_indices],axis=0).std()
                selected_obs_std_=pd.concat([station_info,selected_obs_std],axis=1); selected_obs_std_.columns=['TOWNNAME','value']
                ...
                #calculate the absolute error
                selected_simulate_mae=pd.DataFrame([ErrorIndicator.np_mae(pd.DataFrame(spatiotemporal_result.iloc[new_unique_index[index],3+forecast_timestep-1]).reset_index(drop=True).T,pd.DataFrame(simulate.iloc[new_unique_index[index],-1]).reset_index(drop=True).T) for index in season_indices]).mean()
                selected_simulate_mae_=pd.concat([station_info,selected_simulate_mae.astype(float)],axis=1); selected_simulate_mae_.columns=['TOWNNAME','value']
                ...
                gdf.plot(ax=ax, edgecolor='black', facecolor='none', linewidth=0.5)
                cbar = fig.colorbar(plt.cm.ScalarMappable(norm=norm, cmap=cmap), ax=ax, ticks=np.linspace(min_val, max_val, 10))
                plt.savefig(r"image_result\season\T+%s\forecast_%s(%s).png"%(forecast_timestep,season[season_index],years[year_index]))
                gc.collect()
                ...
                gdf.plot(ax=ax, edgecolor='black', facecolor='none', linewidth=0.5)
                cbar = fig.colorbar(plt.cm.ScalarMappable(norm=norm, cmap=cmap), ax=ax, ticks=np.linspace(min_val, max_val, 10))
                plt.savefig(r"image_result\season\T+%s\obs_%s(%s).png"%(forecast_timestep,season[season_index],years[year_index]))
                gc.collect()
                ...
                gdf.plot(ax=ax, edgecolor='black', facecolor='none', linewidth=0.5)
                cbar = fig.colorbar(plt.cm.ScalarMappable(norm=norm, cmap=cmap), ax=ax, ticks=np.linspace(min_val, max_val, 10))
                plt.savefig(r"image_result\season\T+%s\simulate_%s(%s).png"%(forecast_timestep,season[season_index],years[year_index]))
                gc.collect()        
                ...
                gdf.plot(ax=ax, edgecolor='black', facecolor='none', linewidth=0.5)
                cbar = fig.colorbar(plt.cm.ScalarMappable(norm=norm, cmap=cmap), ax=ax, ticks=np.linspace(min_val, max_val, 10))
                plt.savefig(r"image_result\season\T+%s\std_%s(%s).png"%(forecast_timestep,season[season_index],years[year_index]))
                gc.collect()
                
                #plot error figure
                min_error = 0; max_error=1
                cmap2 = sns.color_palette("Reds", as_cmap=True)
                ...
                gdf.plot(ax=ax, edgecolor='black', facecolor='none', linewidth=0.5)
                cbar = fig.colorbar(plt.cm.ScalarMappable(norm=norm2, cmap=cmap2), ax=ax, ticks=np.linspace(min_error, max_error, 10))
                plt.savefig(r"image_result\season\T+%s\error_%s(%s).png"%(forecast_timestep,season[season_index],years[year_index]))
                gc.collect()
                ...
                gdf.plot(ax=ax, edgecolor='black', facecolor='none', linewidth=0.5)
                cbar = fig.colorbar(plt.cm.ScalarMappable(norm=norm2, cmap=cmap2), ax=ax, ticks=np.linspace(min_error, max_error, 10))
                plt.savefig(r"image_result\season\T+%s\simulate_error_%s(%s).png"%(forecast_timestep,season[season_index],years[year_index]))
                gc.collect()
            ...
            year_obs.to_excel(writer2,sheet_name="%s_obs_mean"%(years[year_index]))
        
            year_std=pd.DataFrame(year_std).T; year_std.columns=season
            year_std=pd.concat([station_info,year_std],axis=1)
            year_std.to_excel(writer2,sheet_name="%s_obs_std"%(years[year_index]))
            ...
            year_forecast.to_excel(writer2,sheet_name="%s_forecast"%(years[year_index]))
            
            year_mae=pd.DataFrame(year_mae).T; year_mae.columns=season
            year_mae=pd.concat([station_info,year_mae],axis=1)
            year_mae.to_excel(writer2,sheet_name="%s_mae"%(years[year_index]))
            ...
            year_simulate.to_excel(writer2,sheet_name="%s_simulate"%(years[year_index]))
            ...
            year_simulate_mae.to_excel(writer2,sheet_name="%s_mae"%(years[year_index]))
    ...
    zero_stas=pd.read_excel(r'result\seasonal_performance(T+%s).xlsx'%forecast_timestep,sheet_name="2020_subcatchment",index_col=0)
    one_stas=pd.read_excel(r'result\seasonal_performance(T+%s).xlsx'%forecast_timestep,sheet_name="2021_subcatchment",index_col=0)
    two_stas=pd.read_excel(r'result\seasonal_performance(T+%s).xlsx'%forecast_timestep,sheet_name="2022_subcatchment",index_col=0)
    three_stas=pd.read_excel(r'result\seasonal_performance(T+%s).xlsx'%forecast_timestep,sheet_name="2023_subcatchment",index_col=0)
...
if __name__ == '__main__':
    ...
    for forecast_timestep in range(1,7):
        seasonal_performance(forecast_timestep)
